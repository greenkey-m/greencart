...
/**
 * Greencart Route Helper
 **/
abstract class GreencartHelperRoute
{
	...
	public static function getLooksRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			...
			// Create the link
			$link = 'index.php?option=com_greencart&view=looks&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_greencart&view=looks';
		}
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('greencart.looks');
	...
	/**
	 * Get the URL route for greencart category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_greencart&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_greencart');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (GreencartHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
		...
		if ($active
			&& $active->component == 'com_greencart'
